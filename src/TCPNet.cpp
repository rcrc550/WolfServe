...
void TcpNet::Addfd(int fd,int et/*是否为边缘触发*/)
{
...
void TcpNet::Epoll_Deal(int ready,pool_t *pool)
{
    ...
    for(i=0; i<ready; i++)
    {
        ...
        if(sockfd == fd)   //客户端建立链接
            m_pool->Producer_add(pool,Deal_Job,NULL);
        else if(epollarr[i].events & EPOLLIN)
        {
            Deletefd(fd);
            m_pool->Producer_add(pool,Now_Recv,(void*)fd);
        }
    }
}

void *TcpNet::Deal_Job(void *arg)
{
    struct sockaddr_in clientaddr;
    int clientsize = sizeof(clientaddr);
    int clientfd;
    char str[_DEF_IPSIZE];
