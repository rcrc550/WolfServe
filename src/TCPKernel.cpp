...
libevent_Net* TcpKernel::m_tcp2= NULL ;
...
int TcpKernel::Open( int port)
{
    ...
    m_tcp = new libevent_Net(&TcpKernel::DealData);

    bool res = m_tcp->InitServer(port  ) ;
     m_tcp2=m_tcp;
    if( !res )
...
void TcpKernel::DealData(/*int clientfd*/struct bufferevent* clientfd,char *szbuf,int nlen)
{
...
void TcpKernel::sendData(bufferevent *clientfd, char *szbuf, int nlen)
{
...
void TcpKernel::SendData(/*int clientfd*/struct bufferevent* clientfd, char *szbuf, int nlen)
{
...
//注册
void TcpKernel::RegisterRq(/*int clientfd*/struct bufferevent* clientfd,char* szbuf,int nlen)//注册到来的回复 主要是看有没有重名重复id的
{
...
//创建房间
void TcpKernel::CreateRoomRq(/*int clientfd*/struct bufferevent* clientfd, char *szbuf, int nlen)
{
...
//加入房间
void TcpKernel::JoinRoomRq(/*int clientfd*/struct bufferevent* clientfd, char *szbuf, int nlen)
{
...
//离开房间
void TcpKernel::LeaveRoomRq(/*int clientfd*/struct bufferevent* clientfd, char *szbuf, int nlen)
{
...
void TcpKernel::WolfLeave(/*int clientfd*/struct bufferevent* clientfd, char *szbuf, int nlen)
{
     //printf("leave no this person\n");
...
void TcpKernel::ReadyWolf(bufferevent *clientfd, char *szbuf, int nlen)
{
       ...
       if(c->m_wolfid==rq->m_roomid&&c->ready==0)
       {
           ...
           if(MapWolfReady.find(rq->m_roomid)==WOLf_NUM)
           {
               ...
               WolfPlayRoom *m_room;
                 if(!MapWolfPlayRoom.IsExist(rq->m_roomid))
                 {
                  m_room=new WolfPlayRoom(&TcpKernel::sendData);
...
void TcpKernel::WolfVote(bufferevent *clientfd, char *szbuf, int nlen)
{
...
void TcpKernel::save(bufferevent *clientfd, char *szbuf, int nlen)
{
...
void TcpKernel::dealpoison(bufferevent *clientfd, char *szbuf, int nlen)
{
...
void TcpKernel::dealwatch(bufferevent *clientfd, char *szbuf, int nlen)
{
...
void TcpKernel::dealNorVote(bufferevent *clientfd, char *szbuf, int nlen)
{
...
void TcpKernel::dealAddfrined(bufferevent *clientfd, char *szbuf, int nlen)
{
...
void TcpKernel::dealMatch(bufferevent *clientfd, char *szbuf, int nlen)
{
...
void TcpKernel::dealinfor(bufferevent *clientfd, char *szbuf, int nlen)
{
...
void TcpKernel::dealgetlist(bufferevent *clientfd, char *szbuf, int nlen)
{
...
void TcpKernel::dealMemberLike(bufferevent *clientfd, char *szbuf, int nlen)
{
    printf("member like\n");
...
void TcpKernel::dealMemberRoom(bufferevent *clientfd, char *szbuf, int nlen)
{
            ...
            for(int i=0;i<c;i++)
            {
                ...
                if((c-myin->myvec[i].time)>4)
                {
                    ...
                    if(myin->m_mapIdToUserInfo.IsExist(myin->myvec[i].id))
                    {
                      ...
                      if(user->m_sockfd!=0)
                      {
                        //bufferevent_free(user->m_sockfd );
                        printf("sockt quit1\n");
                        user->m_sockfd=0;
                      }
                      if(user->m_audiofd!=0)
                      {
                      // bufferevent_free(user->m_audiofd );
                         printf("audio quit2\n");
                          user->m_audiofd=0;
                      }
                      if(user->m_videofd!=0)
                      {
                    //   bufferevent_free(user->m_videofd );
                        printf("vidio quit3\n");
                        user->m_videofd=0;
                      }
...
void TcpKernel::AudioFrameRq(/*int clientfd*/struct bufferevent* clientfd, char *szbuf, int nlen)
{
    char *tmp=szbuf;
    tmp+=sizeof(int);
    //跳过type
...
#include<pthread.h>
#include<unistd.h>
void TcpKernel::Dealalive(bufferevent *clientfd, char *szbuf, int nlen)
{
   // printf("come on\n");
...
//登录
void TcpKernel::LoginRq(/*int clientfd*/struct bufferevent* clientfd ,char* szbuf,int nlen)
{
...
void TcpKernel::VideoFrameRq(/*int clientfd*/struct bufferevent* clientfd, char *szbuf, int nlen)
{
...
//音频注册
void TcpKernel::AudioRegister(/*int clientfd*/struct bufferevent* clientfd, char *szbuf, int nlen)
{
...
//视频注册
void TcpKernel::VideoRegister(/*int clientfd*/struct bufferevent* clientfd, char *szbuf, int nlen)
{
...
void TcpKernel::WolfJoin(bufferevent *clientfd, char *szbuf, int nlen)
{
...
void TcpKernel::WolfCreate(bufferevent *clientfd, char *szbuf, int nlen)
{
